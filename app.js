(function () {
  ...
  const RULES = [
    ...
    "三、起点奖励：经过或到达任意一个起点，都会获得房主设定的金额（默认 ¥3000）；且该奖励会随【新城】加成的一半一起上涨（都市不加成起点，只有新城能让起点变多）。",
    ...
    "五、建筑：7 级——店铺→商铺→旅馆→酒店→大厦→新城→都市，必须按顺序升级，一次最多升 2 级，不能跳级。",
    "六、新城/都市效果：建成 1 座【新城】，全场城市地租永久 +0.5 倍；建成 1 座【都市】，全场城市地租永久 +1.0 倍（新城的两倍）。公式：1 + 新城数×0.5 + 都市数×1.0。",
    "七、过路费 = 基础地租 × 同城地块加成 × 全场（新城+都市）倍率。同城每多拥有 1 个地块，加成按房主设定（默认 +0.2，可改 0.2/0.3/0.4/0.5）。地图格子上直接显示当前实际过路费。",
