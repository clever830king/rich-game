(function () {
  ...
  const RULES = [
    ...
    "三、起点奖励：经过或到达任意一个起点，都会获得房主设定的金额（默认 ¥3000）；新城让起点 +0.25 倍/座，都市让起点 +0.5 倍/座（新城的两倍），新区不加成起点。",
    ...
    "五、建筑：8 级——店铺→商铺→旅馆→酒店→大厦→新城→新区→都市，必须按顺序升级，一次最多升 2 级，不能跳级。",
    "六、新城/新区/都市效果：新城地租 +0.5 倍/座；新区地租 +1.0 倍/座（新城两倍）；都市地租 +2.0 倍/座（新区两倍）。公式：1 + 新城×0.5 + 新区×1.0 + 都市×2.0。",
    "七、过路费 = 基础地租 × 同城地块加成 × 全场（新城+新区+都市）倍率。同城每多拥有 1 个地块，加成按房主设定（默认 +0.2，可改 0.2/0.3/0.4/0.5）。地图格子上直接显示当前实际过路费。",
    "八、卡牌补给站：到达后随机获得一张特殊卡（共 10 种）——逆向卡（指定一人逆向 3 回合）、霸王卡（可抵消 2 次过路费，付钱时选择是否兑现）、升级卡（对自己脚下地块再买1区域或升1~2级）、作弊卡（自选点数 1~12）、爆破卡（自己脚下地块建筑全部归零）、拆除卡（他人商铺级(2级)以上建筑拆 2 级）、建造卡（自己任意地块升 1 级，按原价付费）、暂停卡（指定一人原地跳过一回合）、盗窃卡（偷一人一张卡，无卡抢 10% 钱）、强买卡（付他人地块投入的 1.5 倍，地块连建筑归你）。",
    "九、命运（不可控，抽到立即执行，共 52 张）：按约 40% 获得卡牌、10% 失去卡牌、其余为加钱/减钱的配比随机触发；加钱减钱均按现金的 5%~15% 比例；抽卡时有命运骰子动画。",
    "十、机会（主动把握，共 53 张）：可选「把握机会」或「放弃机会」；把握后掷机会骰子（奇数失败、偶数成功），机会骰子不影响移动，且有滚动动画；加钱减钱均按现金的 5%~15% 比例。",
  ...
  function renderGame() {
    ...
    const sm = EN.startRewardMultiplier(room);
    $("#hudNewtown").textContent = "地租加成 ×" + nt.toFixed(1);
    $("#hudStartMult").textContent = "起点加成 ×" + sm.toFixed(1);
    renderBoard();
    renderPieces();
    renderPlayers();
    renderChat();
  ...
  function landingModal() {
    ...
    if (pend.type === "opportunity") {
      ...
      if (money < (card.invest || 0)) g.disabled = true;
